...
export default function StaffPasswordResetPage() {
  ...
  useEffect(() => {
    setMode(token ? 'complete' : 'request');
  }, [token]);

  const title = useMemo(
    () => mode === 'complete' ? 'Create a new Staff Portal password' : 'Reset your Staff Portal password',
    [mode],
  );
  ...
  async function requestReset(event: React.FormEvent) {
    ...
    setError('');
    try {
      const response = await fetch('/api/staff/auth/password-reset', {
        method: 'POST',
      ...
      setMessage(data.message || 'If an eligible BIMED account exists, a reset link will be sent shortly.');
    } catch (caught) {
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f4f7fb', padding: 24, fontFamily: 'system-ui' }}>
      <section style={{ width: '100%', maxWidth: 520, background: '#fff', padding: 32, borderRadius: 20, boxShadow: '0 14px 44px rgba(15,23,42,.08)', border: '1px solid #e5eaf0' }}>
        <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: 1.4, textTransform: 'uppercase', color: '#0f766e' }}>BIMED Healthcare</div>
        <h1 style={{ margin: '10px 0 8px', color: '#102a43', fontSize: 29 }}>{title}</h1>
        <p style={{ color: '#627d98', lineHeight: 1.55 }}>
          {mode === 'complete'
            ? 'Choose a new password for your BIMED Staff Portal account. Your existing sessions will be signed out for security.'
            : 'Enter the BIMED email address attached to your Staff Portal account. We will send a secure reset link if the account is eligible.'}
        </p>
        ...
        {mode === 'request' ? (
          <form onSubmit={requestReset}>
            <label style={{ display: 'block', fontWeight: 800, marginBottom: 6 }}>BIMED email</label>
            <input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" required placeholder="firstname.lastname@bimedhealthcare.com" style={{ width: '100%', boxSizing: 'border-box', padding: '12px 14px', border: '1px solid #d9e2ec', borderRadius: 10, marginBottom: 14 }} />
