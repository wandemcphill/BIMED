...
type Tab = 'overview' | 'rota' | 'leave' | 'pay' | 'profile' | 'notifications';
...
export default function StaffDashboard() {
  ...
  async function submitLeave(event: FormEvent) { event.preventDefault(); setLeaveBusy(true); setError(''); try { const response = await fetch('/api/staff/rota', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'request_leave', startDate: leaveStart, endDate: leaveEnd, leaveType, reason: leaveReason }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to submit leave request.'); setLeaveStart(''); setLeaveEnd(''); setLeaveReason(''); setLeaveType('annual'); await load(); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to submit leave request.'); } finally { setLeaveBusy(false); } }
  async function cancelLeave(requestId: string) { setLeaveBusy(true); setError(''); try { const response = await fetch('/api/staff/rota', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'cancel_leave', requestId }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to cancel leave request.'); setLeave(data.leaveRequests || []); await load(); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to cancel leave request.'); } finally { setLeaveBusy(false); } }
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f4f7fb', color: '#102a43', fontFamily: 'system-ui' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px' }}>
        <nav style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>{([['overview', 'Overview'], ['rota', 'My Rota'], ['leave', 'Leave'], ['pay', 'Payslips'], ['profile', 'My Profile'], ['notifications', `Notifications${unreadCount ? ` (${unreadCount})` : ''}`]] as Array<[Tab, string]>).map(([id, label]) => <button key={id} onClick={() => setTab(id)} style={{ padding: '10px 15px', borderRadius: 10, border: '1px solid #d9e2ec', background: tab === id ? '#0f766e' : '#fff', color: tab === id ? '#fff' : '#334e68', fontWeight: 800 }}>{label}</button>)}</nav>
        ...
        {tab === 'overview' && <>
          ...
          <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 18, marginTop: 18 }}><div style={card}><div style={{ fontSize: 13, color: '#627d98' }}>Upcoming shifts</div><div style={{ fontSize: 30, fontWeight: 900 }}>{shifts.filter((shift) => shift.status !== 'cancelled').length}</div><button onClick={() => router.push('/staff/rota')} style={linkButton}>Open rota →</button></div><div style={card}><div style={{ fontSize: 13, color: '#627d98' }}>Payslips available</div><div style={{ fontSize: 30, fontWeight: 900 }}>{payslips.length}</div><button onClick={() => setTab('pay')} style={linkButton}>View payslips →</button></div><div style={card}><div style={{ fontSize: 13, color: '#627d98' }}>Pending leave</div><div style={{ fontSize: 30, fontWeight: 900 }}>{leave.filter((item) => item.status === 'pending').length}</div><button onClick={() => setTab('leave')} style={linkButton}>Manage leave →</button></div><div style={card}><div style={{ fontSize: 13, color: '#627d98' }}>Unread notifications</div><div style={{ fontSize: 30, fontWeight: 900 }}>{unreadCount}</div><button onClick={() => setTab('notifications')} style={linkButton}>Open notifications →</button></div></section>
        </>}
        ...
        {tab === 'leave' && <section style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(300px,.9fr)', gap: 18 }}><form onSubmit={submitLeave} style={card}><h2 style={{ marginTop: 0 }}>Request leave</h2><p style={{ color: '#627d98', lineHeight: 1.55 }}>Submit time-off requests from the same staff workspace used for your rota. Approved or pending dates cannot overlap.</p><div style={{ display: 'grid', gap: 12 }}><label style={{ fontWeight: 800, fontSize: 13 }}>Leave type<select value={leaveType} onChange={(e) => setLeaveType(e.target.value)} style={inputStyle}><option value="annual">Annual leave</option><option value="sick">Sick leave</option><option value="family">Family leave</option><option value="unpaid">Unpaid leave</option><option value="other">Other</option></select></label><label style={{ fontWeight: 800, fontSize: 13 }}>Start date<input required type="date" value={leaveStart} onChange={(e) => setLeaveStart(e.target.value)} style={inputStyle} /></label><label style={{ fontWeight: 800, fontSize: 13 }}>End date<input required type="date" value={leaveEnd} onChange={(e) => setLeaveEnd(e.target.value)} style={inputStyle} /></label><label style={{ fontWeight: 800, fontSize: 13 }}>Reason<textarea value={leaveReason} onChange={(e) => setLeaveReason(e.target.value)} maxLength={2000} rows={4} style={{ ...inputStyle, resize: 'vertical' }} /></label><button disabled={leaveBusy} type="submit" style={primaryButton}>{leaveBusy ? 'Submitting…' : 'Request leave'}</button></div></form><section style={card}><h2 style={{ marginTop: 0 }}>My leave history</h2>{leave.length === 0 ? <p style={{ color: '#627d98' }}>No leave requests recorded.</p> : leave.map((item) => <div key={item.id} style={{ padding: '12px 0', borderTop: '1px solid #edf2f7' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><strong>{item.leave_type}</strong><span style={{ fontSize: 12, fontWeight: 900 }}>{item.status}</span></div><div style={{ fontSize: 13, color: '#627d98', marginTop: 4 }}>{item.start_date} → {item.end_date} · {item.total_days ?? '—'} day(s)</div>{item.reason && <div style={{ marginTop: 5, fontSize: 12, color: '#627d98' }}>{item.reason}</div>}{item.review_note && <div style={{ marginTop: 5, fontSize: 12, color: '#486581' }}>{item.review_note}</div>}{['pending','approved'].includes(item.status) && <button disabled={leaveBusy} onClick={() => void cancelLeave(item.id)} style={{ ...secondaryButton, marginTop: 8 }}>Cancel request</button>}</div>)}</section></section>}
