...
export default function StaffDashboard() {
  const router = useRouter(); const [staff, setStaff] = useState<Staff | null>(null); const [shifts, setShifts] = useState<Shift[]>([]); const [requests, setRequests] = useState<any[]>([]); const [payslips, setPayslips] = useState<Payslip[]>([]); const [notifications, setNotifications] = useState<Notice[]>([]); const [tab, setTab] = useState<Tab>('overview'); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); const [photoBusy, setPhotoBusy] = useState(false); const [pps, setPps] = useState(''); const [address, setAddress] = useState(''); const [phone, setPhone] = useState('');
  ...
  async function load() {
    ...
    setStaff(me.staff); setPps(me.staff.pps_number || ''); setAddress(me.staff.address_line_1 || ''); setPhone(me.staff.phone || ''); setShifts(rota.shifts || []); setRequests(rota.requests || []); setPayslips(pay.payslips || []); setNotifications(notice.notifications || []);
  }
  useEffect(() => { void load(); }, []);
  ...
  async function uploadPhoto(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; setPhotoBusy(true); setError(''); try { const formData = new FormData(); formData.append('photo', file); const response = await fetch('/api/staff/me', { method: 'POST', body: formData }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Unable to upload photograph.'); setStaff(data.staff); } catch (photoError) { setError(photoError instanceof Error ? photoError.message : 'Unable to upload photograph.'); } finally { setPhotoBusy(false); event.target.value = ''; } }
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px' }}>
        ...
        {tab === 'overview' && <>
          ...
          <section style={{ marginTop: 18 }}><h2 style={{ margin: '0 0 10px' }}>Your BIMED tools</h2><p style={{ margin: '0 0 14px', color: '#627d98' }}>Your account has the same core workforce access. Recruitment-linked staff also have their onboarding workspace.</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 12 }}>
            ...
            {isIntake ? <ToolCard title="Onboarding" text="Review your recruitment-linked onboarding readiness and outstanding requirements." href="/staff/onboarding" /> : <ToolCard title="Work profile" text="Your internal BIMED employment details and workplace identity are managed here." onClick={() => setTab('profile')} />}
          </div></section>
