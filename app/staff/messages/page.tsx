...
export default function BimedMessagesPage() {
  ...
  const [to, setTo] = useState('__bimed_admin__');
  ...
  async function load() {
    ...
    setMailbox(data.mailbox); setStaffType(data.staffType || ''); setConversations(data.conversations || []);
  }
  ...
  async function startConversation(event: FormEvent) {
    event.preventDefault(); if (!draft.trim()) return; setBusy(true); setError('');
    try {
      ...
      setDraft(''); await load(); await openConversation(data.conversation.id);
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f4f7fb', color: '#102a43', fontFamily: 'system-ui' }}>
      <header style={{ background: '#fff', borderBottom: '1px solid #e5eaf0', padding: '18px 5vw', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
        <div><div style={{ color: '#0f766e', fontSize: 12, fontWeight: 900, letterSpacing: 1.4 }}>BIMED HEALTHCARE</div><h1 style={{ margin: '4px 0 0', fontSize: 26 }}>BIMED Messages</h1><div style={{ marginTop: 5, color: '#627d98', fontSize: 13 }}>{staffType === 'recruitment_intake' ? 'Recruitment intake · onboarding and workforce communications' : 'Internal BIMED staff · workplace communications'}</div></div>
        ...
        <aside style={{ background: '#fff', border: '1px solid #e5eaf0', borderRadius: 16, padding: 14 }}>
          <form onSubmit={startConversation} style={{ borderBottom: '1px solid #edf2f7', paddingBottom: 14, marginBottom: 14 }}>
            ...
            <select value={to} onChange={(e) => setTo(e.target.value)} style={{ width: '100%', padding: 10, border: '1px solid #cbd5e1', borderRadius: 10, boxSizing: 'border-box', marginBottom: 8 }}>
              <option value="__bimed_admin__">BIMED Admin / HR</option>
            </select>
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Write a message to BIMED Admin / HR…" rows={3} style={{ width: '100%', padding: 10, border: '1px solid #cbd5e1', borderRadius: 10, boxSizing: 'border-box', resize: 'vertical' }} />
            <button disabled={busy || !draft.trim()} style={{ width: '100%', marginTop: 8, padding: 10, border: 0, borderRadius: 10, background: '#0f766e', color: '#fff', fontWeight: 900 }}>{busy ? 'Sending…' : 'Send to BIMED Admin'}</button>
          </form>
