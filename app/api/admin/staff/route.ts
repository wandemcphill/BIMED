...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'status') {
      ...
      if (target.application_id && body.status === 'suspended' && target.portal_restriction_reason !== 'accommodation_nonpayment') return NextResponse.json({ error: 'Use the dedicated portal restriction action for recruitment-linked new recruits. Suspension of a post-contract recruit is reserved for accommodation non-payment.' }, { status: 409 });
