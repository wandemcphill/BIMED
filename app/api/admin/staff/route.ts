...
export async function POST(request: NextRequest) {
  ...
  try {
    if (body.action === 'create_internal') {
      ...
      if (!fullName || !email || !jobTitle || !department || !employmentType) return NextResponse.json({ error: 'Full name, email, job title, department and employment type are required.' }, { status: 400 });
      if (!/^[^\s@,;<>]+@[^\s@,;<>]+\.[^\s@,;<>]+$/.test(email)) return NextResponse.json({ error: 'Enter a valid staff email address.' }, { status: 400 });
      if (!INTERNAL_DEPARTMENTS.includes(department as any)) return NextResponse.json({ error: 'Invalid BIMED department.' }, { status: 400 }); if (!EMPLOYMENT_TYPES.includes(employmentType as any)) return NextResponse.json({ error: 'Invalid employment type.' }, { status: 400 });
      if (normalizeRecruitmentRole(jobTitle)) return NextResponse.json({ error: 'Care and clinical roles must be created through the recruitment pathway.' }, { status: 400 });
    ...
    if (body.action === 'restrict_portal') {
      ...
      const { data: target } = await client.from('recruitment_staff').select('id,bimed_id,full_name,email,status,application_id,session_version').eq('id', body.staffId).maybeSingle();
      if (!target) return NextResponse.json({ error: 'Staff member not found.' }, { status: 404 });
      if (!target.application_id) return NextResponse.json({ error: 'Portal restriction for this workflow is limited to recruitment-linked new recruits.' }, { status: 400 });
      if (target.status === 'suspended' && target.portal_restriction_reason === 'accommodation_nonpayment') return NextResponse.json({ error: 'This recruit is already restricted for accommodation non-payment.' }, { status: 409 });
      ...
      const { data, error } = await client.from('recruitment_staff').update({ status: 'suspended', portal_previous_status: target.status === 'suspended' ? 'pre_arrival' : target.status, portal_restriction_reason: 'accommodation_nonpayment', portal_restricted_at: now, portal_restricted_by: session.email, portal_restriction_message: message, session_version: (target.session_version || 1) + 1, updated_at: now }).eq('id', target.id).select('*').single();
      if (error || !data) return NextResponse.json({ error: 'Unable to restrict portal access.' }, { status: 500 });
      await createStaffAudit(client, { staffId: target.id, actor: session.email, eventType: 'staff_portal_restricted', metadata: { reason: 'accommodation_nonpayment', previous_status: target.status } });
      return NextResponse.json({ staff: data, message });
    }

    if (body.action === 'reactivate_portal') {
      if (!body.staffId) return NextResponse.json({ error: 'staffId is required.' }, { status: 400 });
      const { data: target } = await client.from('recruitment_staff').select('*').eq('id', body.staffId).maybeSingle();
      if (!target) return NextResponse.json({ error: 'Staff member not found.' }, { status: 404 });
      if (target.status !== 'suspended' || target.portal_restriction_reason !== 'accommodation_nonpayment') return NextResponse.json({ error: 'This staff record is not restricted by the accommodation non-payment workflow.' }, { status: 409 });
      const restoredStatus = ['pre_arrival', 'active', 'on_leave'].includes(target.portal_previous_status) ? target.portal_previous_status : 'pre_arrival'; const now = new Date().toISOString();
      const { data, error } = await client.from('recruitment_staff').update({ status: restoredStatus, portal_restriction_reason: null, portal_restricted_at: null, portal_restricted_by: null, portal_restriction_message: null, portal_previous_status: null, session_version: (target.session_version || 1) + 1, updated_at: now }).eq('id', target.id).select('*').single();
      if (error || !data) return NextResponse.json({ error: 'Unable to reactivate portal access.' }, { status: 500 });
      await createStaffAudit(client, { staffId: target.id, actor: session.email, eventType: 'staff_portal_reactivated', metadata: { restored_status: restoredStatus } });
      return NextResponse.json({ staff: data, message: 'Portal access has been restored. The recruit can sign in again using the same BIMED ID, email and existing profile details.' });
    }
