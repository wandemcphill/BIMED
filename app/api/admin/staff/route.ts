...
export async function POST(request: NextRequest) {
  ...
  let body: {
    action?: string; applicationId?: string; staffId?: string; status?: string;
    fullName?: string; email?: string; phone?: string | null; jobTitle?: string;
    department?: string; employmentType?: string; startDate?: string | null; managerName?: string | null;
    address?: string | null; city?: string | null; county?: string | null; eircode?: string | null; country?: string | null;
  };
  ...
  try {
    if (body.action === 'create_internal') {
      ...
      if (normalizeRecruitmentRole(jobTitle)) {
        return NextResponse.json({ error: 'Care and clinical roles must be created through the recruitment pathway. Use this form only for internal BIMED staff.' }, { status: 400 });
      }
      ...
      const { data: staff, error } = await client.from('recruitment_staff').insert({
        application_id: null,
        bimed_id: bimedId,
        status: 'active',
        full_name: fullName,
      ...
      await createStaffAudit(client, { staffId: staff.id, actor: session.email, eventType: 'internal_staff_created', metadata: { department, job_title: jobTitle, employment_type: employmentType } });
      return NextResponse.json({ staff, activationUrl: createActivationUrl(request, token, staff.email) }, { status: 201 });
    }
    ...
    if (body.action === 'rotate_activation') {
      ...
      return NextResponse.json({ activationUrl: createActivationUrl(request, token, data.email) });
    }
