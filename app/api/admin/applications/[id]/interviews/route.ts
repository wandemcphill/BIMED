...
import { readJsonBody } from '@/lib/input-validation';
...
function parseScheduledAt(value: unknown): { ok: true; iso: string } | { ok: false; error: string } {
  if (typeof value !== 'string' || !value.trim()) return { ok: false, error: 'An interview date and time is required.' };
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) return { ok: false, error: 'The interview date and time is not a valid date.' };
  if (parsed.getTime() < Date.now() - 60_000) return { ok: false, error: 'The interview date and time must be in the future.' };
...
async function loadApplication(client: ReturnType<typeof db>, applicationId: string) {
  const { data, error } = await client.from('recruitment_applications').select('id, full_name, email, role_applied, status, living_in_ireland, country_of_residence, phone, work_permission, submitted_at').eq('id', applicationId).maybeSingle();
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const { data, error } = await db().from('recruitment_interviews').select('*').eq('application_id', applicationId).order('scheduled_at', { ascending: false });
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (!rateLimit.allowed) return NextResponse.json({ error: 'Too many interview requests. Please try again later.' }, { status: 429, headers: rateLimit.retryAfterSeconds ? { 'Retry-After': String(rateLimit.retryAfterSeconds) } : undefined });

  const { id: applicationId } = await context.params;
  const parsed = await readJsonBody(request, 24 * 1024);
  if (!parsed.ok) return NextResponse.json({ error: parsed.error }, { status: 400 });
  const body = parsed.value;
  const scheduledAt = parseScheduledAt(body.scheduled_at);
  if (!scheduledAt.ok) return NextResponse.json({ error: scheduledAt.error }, { status: 400 });
  const meetingLinkRaw = trimText(body.meeting_link);
  if (meetingLinkRaw && !safeUrl(meetingLinkRaw)) return NextResponse.json({ error: 'The meeting link must be a valid http(s) URL.' }, { status: 400 });
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (!interviewId) return NextResponse.json({ error: 'An interview id is required.' }, { status: 400 });
  if (action !== 'reschedule' && action !== 'cancel') return NextResponse.json({ error: 'Action must be either "reschedule" or "cancel".' }, { status: 400 });

  try {
    const client = db();
    const application = await loadApplication(client, applicationId);
    if (!application) return NextResponse.json({ error: 'Application not found.' }, { status: 404 });
    const { data: existing, error: existingError } = await client.from('recruitment_interviews').select('*').eq('id', interviewId).eq('application_id', applicationId).maybeSingle();
    if (existingError) throw existingError;
    if (!existing) return NextResponse.json({ error: 'Interview not found.' }, { status: 404 });
    if (existing.status === 'Cancelled') return NextResponse.json({ error: 'This interview has already been cancelled.' }, { status: 409 });

    if (action === 'cancel') {
      const reason = trimText(body.reason);
      const { data: cancelled, error: cancelError } = await client.from('recruitment_interviews').update({ status: 'Cancelled', cancelled_at: new Date().toISOString(), cancellation_reason: reason, updated_at: new Date().toISOString() }).eq('id', interviewId).select('*').single();
      if (cancelError) throw cancelError;
      await recordRecruitmentAudit(client, { applicationId, eventType: 'interview_cancelled', actor: session.email, metadata: { interview_id: interviewId, had_reason: Boolean(reason) } });
    ...
    const { data: rescheduled, error: rescheduleError } = await client.from('recruitment_interviews').update({
      scheduled_at: scheduledAt.iso,
      ...
      candidate_instructions: body.candidate_instructions === undefined ? existing.candidate_instructions : trimText(body.candidate_instructions, 1000),
      status: 'Rescheduled',
      reschedule_count: nextRevision,
      updated_at: new Date().toISOString(),
    }).eq('id', interviewId).select('*').single();
    if (rescheduleError) throw rescheduleError;
    await recordRecruitmentAudit(client, { applicationId, eventType: 'interview_rescheduled', actor: session.email, metadata: { interview_id: interviewId, previous_scheduled_at: existing.scheduled_at, scheduled_at: rescheduled.scheduled_at } });
    const emails = await sendInterviewRescheduledEmails({ application, interview: rescheduled, previousScheduledAt: existing.scheduled_at, revision: nextRevision }, client);
    return NextResponse.json({ interview: rescheduled, email: emails.candidate });
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', event: 'interview.update_failed', application_id: applicationId, reason: error instanceof Error ? error.message : 'unknown' }));
