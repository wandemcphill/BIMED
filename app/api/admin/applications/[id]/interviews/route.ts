...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const bodyResult = await readJsonBody(request, MAX_JSON_BYTES.interview);
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (action === 'cancel') {
      ...
      const { data: cancelled, error: cancelError } = await client.from('recruitment_interviews').update({
        status: 'Cancelled',
    ...
    await recordRecruitmentAudit(client, {
      ...
      actor: session.email,
      metadata: { interview_id: interviewId, previous_scheduled_at: existing.scheduled_at, scheduled_at: rescheduled.scheduled_at },
    });
