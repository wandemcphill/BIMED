...
function accommodationReady(invoice: any) {
...
export async function GET(request: NextRequest) {
  ...
  const { data: invoice } = await client.from('recruitment_accommodation_invoices').select('id,invoice_number,status,issue_date,due_date,amount_eur').eq('permit_case_id', permit.id).maybeSingle();
  const homeCountry = String(application?.country_of_residence || application?.nationality || '').trim() || null;
  const homeCountrySource = application?.country_of_residence ? 'country_of_residence' : application?.nationality ? 'nationality_fallback' : null;
  const ready = accommodationReady(invoice);

  try {
    if (!homeCountry) throw new Error('Your home country is missing from the recruitment profile. BIMED must update it before travel planning can continue.');
    const origin = await resolveWorldwideHomeAirport(homeCountry);
    ...
    return NextResponse.json({ homeCountry, homeCountrySource, origin, destination: { code: 'DUB', name: 'Dublin Airport', country: 'Ireland' }, permit, invoice, accommodationReady: ready, itinerary: ready ? saved || null : null, pickup: ready ? pickup || null : null });
  } catch (error) {
    return NextResponse.json({ homeCountry, homeCountrySource, origin: null, destination: { code: 'DUB', name: 'Dublin Airport', country: 'Ireland' }, permit, invoice, accommodationReady: ready, itinerary: null, pickup: null, warning: error instanceof Error ? error.message : 'Unable to resolve the home-country airport.' });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const homeCountry = String(application?.country_of_residence || application?.nationality || '').trim();
    if (!homeCountry) throw new Error('Your home country is missing from the recruitment profile. BIMED must update it before a travel request can be submitted.');
    const origin = await resolveWorldwideHomeAirport(homeCountry);
    const result = await submitFlightTravelRequest({ client, staff, permit, origin, travelDate, passengers });
    await createStaffNotification(client, { staffId: staff.id, category: 'travel', title: 'Travel request sent to BIMED', body: `Your economy travel request from ${origin.code} to Dublin for ${travelDate} has been sent to Overseas Recruitment. BIMED will select, pay for and book the actual flight after immigration clearance, and BIMED will arrange and pay for the Dublin airport pickup.`, actionUrl: '/staff/travel' });
    await createStaffAudit(client, { staffId: staff.id, actor: session.email, eventType: 'flight_travel_request_submitted', metadata: { origin: origin.code, destination: 'DUB', travel_date: travelDate, passenger_count: passengers.length, provider: 'BIMED booking workflow', home_country: homeCountry, accommodation_invoice_id: invoice.id, accommodation_invoice_status: invoice.status } });
