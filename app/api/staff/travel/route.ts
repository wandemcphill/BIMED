...
import { createVirtualFlightItinerary, getHomeDepartureAirport } from '@/lib/flight-travel';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const origin = await getHomeDepartureAirport(String(application?.country_of_residence || ''));
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const origin = await getHomeDepartureAirport(application.country_of_residence);
