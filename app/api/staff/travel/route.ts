...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ homeCountry, homeCountrySource, origin, destination: { code: 'DUB', name: 'Dublin Airport', country: 'Ireland' }, permit, itinerary: saved || null, pickup: pickup || null });
  } catch (error) {
    return NextResponse.json({ homeCountry, homeCountrySource, origin: null, destination: { code: 'DUB', name: 'Dublin Airport', country: 'Ireland' }, permit, itinerary: null, pickup: null, warning: error instanceof Error ? error.message : 'Unable to resolve the home-country airport.' });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await createStaffAudit(client, { staffId: staff.id, actor: session.email, eventType: 'flight_travel_request_submitted', metadata: { origin: origin.code, destination: 'DUB', travel_date: travelDate, passenger_count: passengers.length, provider: 'BIMED booking workflow', home_country: homeCountry } });
