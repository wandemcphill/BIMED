...
import AccommodationInvoiceActions from '@/components/AccommodationInvoiceActions';
...
export default async function AccommodationInvoicePage({ params, searchParams }: Props) {
  ...
  const showActions = !receipt && ['issued', 'payment_reported', 'cancellation_requested'].includes(invoice.status);

  return <main style={{ minHeight: '100vh', background: '#eef2f5', padding: 20, fontFamily: 'Arial,sans-serif' }}>
    {showActions ? <AccommodationInvoiceActions token={token} status={invoice.status} invoiceNumber={invoice.invoice_number} /> : null}
