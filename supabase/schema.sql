...
-- Defense in depth: service-role server access continues to work, while anonymous
-- and browser client roles are denied because no permissive policies are defined.
...
-- Atomically consume one invitation and create the application. The row lock prevents
-- concurrent requests from reusing a single invitation.
create or replace function consume_and_create_recruitment_application(
  p_token_hash text,
  p_payload jsonb
)
returns uuid
...
as $$
declare
  invite_row recruitment_invites%rowtype;
  application_id uuid;
begin
  ...
  insert into recruitment_applications (
    ...
    p_payload->>'address', p_payload->>'role_applied', p_payload->>'employment_type', p_payload->>'availability',
    nullif(p_payload->>'start_date','')::date, p_payload->>'driving_licence', p_payload->>'vehicle_access',
    p_payload->>'care_experience', p_payload->>'qualifications', p_payload->>'training',
    p_payload->>'professional_experience', p_payload->>'employment_history', p_payload->>'employment_gaps',
    p_payload->>'professional_references', p_payload->>'living_in_ireland', p_payload->>'current_country',
    p_payload->>'work_permission', p_payload->>'requires_employment_permit', p_payload->>'international_experience',
    p_payload->>'relocation_readiness', coalesce(p_payload->'supporting_documents','[]'::jsonb), true, 'Submitted', now()
...
-- Purge according to the documented retention policy. Defaults are intentionally explicit
-- and can be overridden by the scheduled job.
create or replace function purge_recruitment_data(
  p_application_retention_days integer default 730,
  p_email_log_retention_days integer default 180,
  p_rate_limit_retention_days integer default 3,
  p_invite_retention_days integer default 90,
...
declare
  ...
  invites_deleted integer := 0;
  audit_deleted integer := 0;
  interviews_deleted integer := 0;
  email_logs_deleted integer := 0;
  sessions_deleted integer := 0;
  resets_deleted integer := 0;
  rate_limits_deleted integer := 0;
begin
  delete from recruitment_audit_log where created_at < now() - make_interval(days => greatest(p_application_retention_days, 1));
  get diagnostics audit_deleted = row_count;

  delete from recruitment_interviews where updated_at < now() - make_interval(days => greatest(p_application_retention_days, 1));
  get diagnostics interviews_deleted = row_count;

  delete from recruitment_applications where submitted_at < now() - make_interval(days => greatest(p_application_retention_days, 1));
  get diagnostics applications_deleted = row_count;

  delete from recruitment_invites
  where (used_at is not null and used_at < now() - make_interval(days => greatest(p_invite_retention_days, 1)))
     or (used_at is null and expires_at is not null and expires_at < now() - make_interval(days => greatest(p_invite_retention_days, 1)));
  get diagnostics invites_deleted = row_count;

  delete from recruitment_email_log where created_at < now() - make_interval(days => greatest(p_email_log_retention_days, 1));
  get diagnostics email_logs_deleted = row_count;

  delete from recruitment_admin_sessions where expires_at < now() or revoked_at < now() - interval '7 days';
  get diagnostics sessions_deleted = row_count;

  delete from recruitment_admin_password_resets where expires_at < now() - make_interval(days => greatest(p_password_reset_retention_days, 1));
  get diagnostics resets_deleted = row_count;

  delete from recruitment_rate_limits where updated_at < now() - make_interval(days => greatest(p_rate_limit_retention_days, 1));
  ...
  return jsonb_build_object(
    ...
    'password_resets_deleted', resets_deleted,
