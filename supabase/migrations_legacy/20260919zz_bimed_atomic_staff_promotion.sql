...
begin
  ...
  select string_agg(item.title, ', ' order by item.item_key)
  into missing_pre_access
  from public.recruitment_onboarding_checklist item
  where item.application_id = app_row.id
    and item.required
    and item.item_key in ('identity_verified', 'qualification_evidence_verified')
    and item.status not in ('completed', 'waived');

  if nullif(missing_pre_access, '') is not null then
    raise exception using
      errcode='P0001',
      message='PRE_ACCESS_VERIFICATION_BLOCKED',
      detail=format('Complete the following before staff creation: %s', missing_pre_access);
  end if;
