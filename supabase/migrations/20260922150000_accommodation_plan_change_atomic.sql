-- Allow an unpaid accommodation selection to be replaced before the employment-permit journey starts.
-- The existing invoice row is reused so the permit-case one-to-one invoice constraint remains intact.
-- The prior selection is retained inside arrangement_snapshot and the staff audit log.
