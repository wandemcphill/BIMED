-- Candidate accommodation-fee rejection with a 24-hour reversal window.
-- The candidate's rejection does not immediately withdraw the application.
-- BIMED automatically finalises it after 24 hours unless the candidate revokes the cancellation.
...
alter table public.recruitment_staff_permit_cases
  ...
  add column if not exists cancellation_previous_application_status text,
  add column if not exists cancellation_previous_invoice_status text,
  add column if not exists cancellation_previous_payment_status text,
  ...
  add column if not exists sponsorship_withdrawn_at timestamptz,
  add column if not exists employment_contract_voided_at timestamptz;

alter table public.recruitment_contract_signatures
  add column if not exists voided_at timestamptz,
  add column if not exists voided_reason text;

create index if not exists recruitment_staff_permit_cases_cancellation_due_idx
  on public.recruitment_staff_permit_cases(cancellation_deadline_at)
  where cancellation_requested_at is not null
    and cancellation_revoked_at is null
...
begin
  ...
  if not found then raise exception 'ACCOMMODATION_INVOICE_NOT_FOUND'; end if;
  if v_invoice.status <> 'cancellation_requested' then
    raise exception 'ACCOMMODATION_INVOICE_NOT_RESTORABLE';
  end if;
  ...
  if v_restored_invoice_status not in ('issued', 'payment_reported') then
    v_restored_invoice_status := 'issued';
  end if;

  v_restored_payment_status := coalesce(v_permit.cancellation_previous_payment_status, 'not_due');

  update public.recruitment_accommodation_invoices
  set status = v_restored_invoice_status,
      updated_at = v_now
  where id = v_invoice.id;
...
begin
  ...
  loop
    ...
    if not found or v_staff.application_id is null then
      continue;
    end if;

    if v_staff.portal_restriction_reason = 'sponsorship_cancellation'
       and v_staff.status = 'suspended' then
      -- Already finalised by an earlier run.
      update public.recruitment_staff_permit_cases
...
-- Replace any prior copy of the scheduler job so applying the migration stays idempotent.
