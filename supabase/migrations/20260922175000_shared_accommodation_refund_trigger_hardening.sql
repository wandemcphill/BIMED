-- Correct the primary shared-accommodation refund trigger to use the route selected in the current acknowledgement request.
-- Repair existing one-month shared primary records that retained the old default trigger.
...
update public.recruitment_staff_permit_cases
set accommodation_refund_trigger = 'one_month_accommodation_expiry',
    accommodation_selection_snapshot = coalesce(accommodation_selection_snapshot, '{}'::jsonb)
      || jsonb_build_object('accommodation_refund_trigger', 'one_month_accommodation_expiry', 'refund_trigger', 'one_month_accommodation_expiry'),
    updated_at = clock_timestamp()
where accommodation_plan = 'one_month_shared_625'
  and permit_submission_route = 'candidate_or_agency'
  and accommodation_refund_trigger = 'successful_three_month_probation';

update public.recruitment_accommodation_invoices i
set arrangement_snapshot = coalesce(i.arrangement_snapshot, '{}'::jsonb)
  || jsonb_build_object('accommodation_refund_trigger', 'one_month_accommodation_expiry', 'refund_trigger', 'one_month_accommodation_expiry'),
    updated_at = clock_timestamp()
from public.recruitment_staff_permit_cases p
where i.permit_case_id = p.id
  and p.accommodation_plan = 'one_month_shared_625'
  and p.permit_submission_route = 'candidate_or_agency';
