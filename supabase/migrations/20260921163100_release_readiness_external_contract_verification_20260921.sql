...
as $$
  with required_tables(name) as (
    values
      ...
      ('recruitment_external_contract_verifications')
  ),
  required_functions(signature) as (
    values
      ...
      ('public.bimed_promote_application_to_staff(uuid,text,text,text,text,text,text,timestamptz,boolean,date,date)'::text),
      ('public.bimed_request_staff_leave(uuid,text,date,date,text,text)'::text),
      ('public.bimed_record_external_contract_verification(uuid,text,text,text)'::text)
  ),
  missing_tables as (
  ...
  select jsonb_build_object(
    'ok',
      ...
      and (select shift_fk_ok from relation_state)
      and (select onboarding_ack_index_ok from relation_state),
    ...
    'shift_fk_ok', (select shift_fk_ok from relation_state),
    'onboarding_ack_index_ok', (select onboarding_ack_index_ok from relation_state),
