-- Canonical formal employment dates for recruitment-linked documentation.
-- Candidate-submitted application.start_date remains source data for availability/preference;
-- formal documents and recruitment-linked staff use the BIMED contractual dates.

insert into public.recruitment_audit_log (application_id, event_type, actor, metadata)
select
  application_id,
  'formal_dates_normalized',
  'system:2026-09-17-formal-dates',
  jsonb_build_object(
    'entity', 'recruitment_contract_signatures',
    'signature_id', id,
    'doc_type', doc_type,
    'old_start_date', start_date,
    'new_start_date', '2027-01-11'
  )
from public.recruitment_contract_signatures
where start_date is distinct from date '2027-01-11';

update public.recruitment_contract_signatures
set start_date = date '2027-01-11'
where start_date is distinct from date '2027-01-11';

insert into public.recruitment_staff_audit_log (staff_id, action, event_type, actor, metadata)
select
  s.id,
  'formal_dates_normalized',
  'formal_dates_normalized',
  'system:2026-09-17-formal-dates',
  jsonb_build_object(
    'old_employment_start_date', s.employment_start_date,
    'old_employment_end_date', s.employment_end_date,
    'new_employment_start_date', '2027-01-11',
    'new_employment_end_date', '2029-01-10'
  )
from public.recruitment_staff s
where s.application_id is not null
  and s.status = 'pre_arrival'
  and (
    s.employment_start_date is distinct from date '2027-01-11'
    or s.employment_end_date is distinct from date '2029-01-10'
  );

update public.recruitment_staff
set employment_start_date = date '2027-01-11',
    employment_end_date = date '2029-01-10',
    updated_at = now()
where application_id is not null
  and status = 'pre_arrival'
  and (
    employment_start_date is distinct from date '2027-01-11'
    or employment_end_date is distinct from date '2029-01-10'
  );

-- The accommodation acknowledgement RPC previously inserted only event_type into
-- recruitment_staff_audit_log even though the action column is now NOT NULL.
-- Keep both audit dimensions populated so the acknowledgement can complete atomically.
