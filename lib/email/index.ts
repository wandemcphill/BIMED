...
export async function sendApplicationReceivedEmails(application: ApplicationEmailRecord, client?: SupabaseClient | null): Promise<{ candidate: SendResult; internal: SendResult[] }> {
  ...
  const candidate = await sendTransactionalEmail({
    ...
    emailType: 'application_received', dedupeKey: `application_received:${application.id}`, applicationId: application.id, client, replyTo: supportingDocumentsEmail(application),
  });
...
const CANDIDATE_STATUS_GUIDANCE: Record<string, string> = {
  ...
  Hired: 'Congratulations. You have been hired by Bimed Healthcare. Your permanent Staff Portal account is being prepared. Check your welcome email for your BIMED ID, BIMED email, activation link and instructions.',
...
export async function sendApplicationStatusUpdateEmails(input: { application: ApplicationEmailRecord; previousStatus: string; status: string; actor: string; notifyCandidate?: boolean }, client?: SupabaseClient | null): Promise<{ candidate: SendResult | null; internal: SendResult[] }> {
  ...
  const candidate = notifyCandidate
    ? await sendTransactionalEmail({
        ...
        emailType: 'application_status_update', dedupeKey: `application_status_update:${application.id}:${status}`, dedupeWindowMs: DUPLICATE_WINDOW_MS, applicationId: application.id, client, replyTo: supportingDocumentsEmail(application),
      })
    : null;
...
export async function sendInterviewInvitationEmails(input: InterviewEmailInput, client?: SupabaseClient | null): Promise<{ candidate: SendResult; internal: SendResult[] }> {
  ...
  const internal = await sendInterviewAdminNotifications('scheduled', input, client); return { candidate, internal };
}

export async function sendInterviewRescheduledEmails(input: InterviewEmailInput, client?: SupabaseClient | null): Promise<{ candidate: SendResult; internal: SendResult[] }> {
  const { application, interview } = input;
  ...
  const internal = await sendInterviewAdminNotifications('rescheduled', input, client); return { candidate, internal };
}

export async function sendInterviewCancelledEmails(input: InterviewEmailInput, client?: SupabaseClient | null): Promise<{ candidate: SendResult; internal: SendResult[] }> {
  const { application, interview } = input;
  ...
  const internal = await sendInterviewAdminNotifications('cancelled', input, client); return { candidate, internal };
}

async function sendInterviewAdminNotifications(action: 'scheduled' | 'rescheduled' | 'cancelled', input: InterviewEmailInput, client?: SupabaseClient | null): Promise<SendResult[]> {
  const { application, interview } = input;
...
export async function sendContractSignedNotificationEmails(input: { application: ApplicationEmailRecord; signedName: string; signedAtLabel: string; signatureId: string }, client?: SupabaseClient | null): Promise<SendResult[]> {
  const content = adminContractSignedEmail({ candidateName: input.application.full_name, role: input.application.role_applied, applicationId: input.application.id, signedName: input.signedName, signedAtLabel: input.signedAtLabel, adminRecordUrl: adminRecordUrl(input.application.id) });
  return Promise.all([recruitmentContacts.admin].map((recipient) => sendTransactionalEmail({ to: recipient, content, emailType: 'admin_contract_signed', dedupeKey: `admin_contract_signed:${input.signatureId}:${recipient}`, applicationId: input.application.id, client })));
}

export async function sendDocumentReadyToSignEmail(input: { application: ApplicationEmailRecord; documentLabel: string; signUrl: string; signatureId: string }, client?: SupabaseClient | null): Promise<SendResult> {
  return sendTransactionalEmail({ to: input.application.email, content: documentReadyToSignEmail({ candidateName: input.application.full_name, documentLabel: input.documentLabel, role: input.application.role_applied, applicationId: input.application.id, signUrl: input.signUrl }), emailType: 'document_ready_to_sign', dedupeKey: `document_ready_to_sign:${input.signatureId}`, applicationId: input.application.id, client, replyTo: recruitmentContacts.ireland });
}
...
export async function sendDocumentSignedNotificationEmails(input: { application: ApplicationEmailRecord; documentLabel: string; signedName: string; signedAtLabel: string; signatureId: string }, client?: SupabaseClient | null): Promise<SendResult[]> {
  const content = adminDocumentSignedEmail({ candidateName: input.application.full_name, documentLabel: input.documentLabel, role: input.application.role_applied, applicationId: input.application.id, signedName: input.signedName, signedAtLabel: input.signedAtLabel, adminRecordUrl: adminRecordUrl(input.application.id) });
  return Promise.all([recruitmentContacts.admin].map((recipient) => sendTransactionalEmail({ to: recipient, content, emailType: 'admin_document_signed', dedupeKey: `admin_document_signed:${input.signatureId}:${recipient}`, applicationId: input.application.id, client })));
}

export async function sendAdminPasswordResetEmail(input: { to: string; displayName: string; resetUrl: string; expiryMinutes: number }, client?: SupabaseClient | null): Promise<SendResult> {
  return sendTransactionalEmail({ to: input.to, content: adminPasswordResetEmail({ displayName: input.displayName, resetUrl: input.resetUrl, expiryMinutes: input.expiryMinutes }), emailType: 'admin_password_reset', client, replyTo: recruitmentContacts.admin });
}
