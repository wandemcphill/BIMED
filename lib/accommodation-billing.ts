...
export const ACCOMMODATION_BANK_VERIFICATION_TIMING = 'Bank-transfer payments to WebGeek Technologies are normally verified within 1–3 working days. International payments may require additional banking checks.';
export const ACCOMMODATION_ONLINE_PAYMENT_TIMING = 'International payments made through the online payment link can take up to 15 working days to be received and settled.';
...
export function invoiceHtml(input: { invoice: any; staff: any; account?: any; publicUrl: string }) {
  // Always use the current canonical payment account. Historical invoice snapshots must not reintroduce retired payment instructions.
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${e(input.invoice.invoice_number)} · BIMED Healthcare</title>${documentStyles()}</head><body>
    <main class="sheet"><div class="topline"></div><div class="inner">
      ...
      <section class="payment"><h2>Payment details</h2><div class="payment-grid">${accountRows.map(([label, value]) => `<div class="payment-row"><span>${e(label)}</span><strong>${e(value)}</strong></div>`).join('')}</div>
        <div class="notice"><strong>Bank transfer verification:</strong> ${e(ACCOMMODATION_BANK_VERIFICATION_TIMING)}</div>
        <div class="notice"><strong>Online payment processing:</strong> ${e(ACCOMMODATION_ONLINE_PAYMENT_TIMING)}</div>
      </section>
      <section class="payment-card">
        <h3>Pay this invoice online</h3>
        <p>Use the secure payment link below. The invoice amount remains the contractual EUR amount shown above.</p>
