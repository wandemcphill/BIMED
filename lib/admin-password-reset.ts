...
export async function createAdminPasswordReset(client: SupabaseClient, email: string, requestedIp?: string | null): Promise<{ token: string; admin: AdminUserRow; expiresAt: Date } | null> {
  ...
  const { error: insertError } = await client.from('recruitment_admin_password_resets').insert({ admin_user_id: admin.id, token_hash: hashToken(token), expires_at: expiresAt.toISOString(), requested_ip: requestedIp || null });
...
export async function consumeAdminPasswordReset(client: SupabaseClient, token: string, newPassword: string): Promise<ResetConsumeResult> {
  ...
  const { data: claimed, error: claimError } = await client.from('recruitment_admin_password_resets').update({ used_at: new Date().toISOString() }).eq('id', resetRow.id).is('used_at', null).select('id');
  if (claimError) throw claimError;
  if (!claimed || claimed.length === 0) return { ok: false, reason: 'used' };

  const { error: updateError } = await client.from('recruitment_admin_users').update({ password_hash: hashAdminPassword(newPassword), updated_at: new Date().toISOString() }).eq('id', admin.id);
  if (updateError) throw updateError;

  const { error: revokeError } = await client.from('recruitment_admin_sessions').update({ revoked_at: new Date().toISOString() }).eq('admin_user_id', admin.id).is('revoked_at', null);
