...
export async function consumeAdminPasswordReset(client: SupabaseClient, token: string, newPassword: string): Promise<ResetConsumeResult> {
  ...
  const { data: admin, error: adminError } = await client.from('recruitment_admin_users').select('id, email, active, session_version').eq('id', resetRow.admin_user_id).maybeSingle();
  ...
  const nextSessionVersion = Math.max(1, Number(admin.session_version || 1) + 1);
